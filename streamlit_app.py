...
with st.sidebar:
    ...
    st.caption("Remover após validação")
    
    if st.button("🧪 Testar Detecção de Conversão", type="secondary"):
        st.markdown("---")
        st.write("**Iniciando teste...**")
        
        # Primeiro: criar registro fake no HISTORICO
        try:
            conn = get_gsheets_connection()
            df_hist = conn.read(worksheet="HISTORICO", ttl=0)
            
            # Adicionar cliente fake no histórico com valor/compras ANTES
            cliente_teste = {
                'Data de contato': '15/12/2025',
                'Nome': 'TESTE João Silva',
                'Classificação': 'Promissor',
                'Valor': 250.00,
                'Telefone': '11999999999',
                'Compras': 2,
                'Relato da conversa': 'Cliente interessado em produtos',
                'Follow up': 'Enviar proposta',
                'Data de chamada': '17/12/2025',
                'Observação': 'Teste de conversão',
                'Data de conclusão': '16/12/2025 10:00'
            }
            ...
            if not df_hist[df_hist['Nome'] == 'TESTE João Silva'].empty:
                ...
                conn.update(worksheet="HISTORICO", data=df_hist_novo)
                st.success("✅ Cliente de teste adicionado ao HISTORICO")
