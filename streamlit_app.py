...
# ========== BOTÃO DE TESTE MASTER - POPULAR TUDO ==========
with st.sidebar:
    st.markdown("---")
    st.markdown("### 🧪 TESTE COMPLETO DO SISTEMA")
    st.caption("Popula todas as abas com dados de teste")
    
    if st.button("🚀 POPULAR TUDO COM DADOS DE TESTE", type="primary", use_container_width=True):
        st.markdown("---")
        st.write("**🔄 Iniciando população completa...**")
        ...
        try:
            ...
            # ========== 1. POPULAR LOG_CHECKINS ==========
            status.write("📝 1/5 - Populando LOG_CHECKINS...")
            progresso.progress(0.2)
            ...
            checkins_teste = [
                {
                    'ID_Checkin': gerar_id_unico('CHK'),
                    'Data_Checkin': '15/12/2025 10:30',
                    'Nome_Cliente': 'João Silva',
                    'Telefone': '11999999001',
                    'Classificacao_Cliente': 'Novo',
                    'Valor_Cliente_Antes': 100.00,
                    'Compras_Cliente_Antes': 1,
                    'Respondeu': 'SIM',
                    'Relato_Resumo': 'Cliente interessado em produtos premium',
                    'Criado_Por': 'Teste',
                    'Dia_Semana': 'Domingo',
                    'Hora_Checkin': '10:30'
                },
                {
                    'ID_Checkin': gerar_id_unico('CHK'),
                    'Data_Checkin': '16/12/2025 14:20',
                    'Nome_Cliente': 'Maria Santos',
                    'Telefone': '11999999002',
                    'Classificacao_Cliente': 'Promissor',
                    'Valor_Cliente_Antes': 250.00,
                    'Compras_Cliente_Antes': 2,
                    'Respondeu': 'SIM',
                    'Relato_Resumo': 'Follow-up de proposta enviada',
                    'Criado_Por': 'Teste',
                    'Dia_Semana': 'Segunda',
                    'Hora_Checkin': '14:20'
                },
                {
                    'ID_Checkin': gerar_id_unico('CHK'),
                    'Data_Checkin': '17/12/2025 09:15',
                    'Nome_Cliente': 'Pedro Costa',
                    'Telefone': '11999999003',
                    'Classificacao_Cliente': 'Leal',
                    'Valor_Cliente_Antes': 500.00,
                    'Compras_Cliente_Antes': 5,
                    'Respondeu': 'SIM',
                    'Relato_Resumo': 'Cliente satisfeito com última compra',
                    'Criado_Por': 'Teste',
                    'Dia_Semana': 'Terça',
                    'Hora_Checkin': '09:15'
                },
                {
                    'ID_Checkin': gerar_id_unico('CHK'),
                    'Data_Checkin': '17/12/2025 11:00',
                    'Nome_Cliente': 'Ana Lima',
                    'Telefone': '11999999004',
                    'Classificacao_Cliente': 'Dormente',
                    'Valor_Cliente_Antes': 150.00,
                    'Compras_Cliente_Antes': 1,
                    'Respondeu': 'NAO',
                    'Relato_Resumo': 'Tentativa de reativação',
                    'Criado_Por': 'Teste',
                    'Dia_Semana': 'Terça',
                    'Hora_Checkin': '11:00'
                },
                {
                    'ID_Checkin': gerar_id_unico('CHK'),
                    'Data_Checkin': '17/12/2025 15:45',
                    'Nome_Cliente': 'Carlos Alves',
                    'Telefone': '11999999005',
                    'Classificacao_Cliente': 'Campeão',
                    'Valor_Cliente_Antes': 1200.00,
                    'Compras_Cliente_Antes': 10,
                    'Respondeu': 'SIM',
                    'Relato_Resumo': 'Cliente VIP, sempre engajado',
                    'Criado_Por': 'Teste',
                    'Dia_Semana': 'Terça',
                    'Hora_Checkin': '15:45'
                }
            ...
            df_log_atualizado = pd.concat([df_log_checkins, pd.DataFrame(checkins_teste)], ignore_index=True)
            conn.update(worksheet="LOG_CHECKINS", data=df_log_atualizado)
            st.success("✅ LOG_CHECKINS populado com 5 registros")
            
            time.sleep(0.5)
            
            # ========== 2. POPULAR LOG_CONVERSOES ==========
            status.write("💰 2/5 - Populando LOG_CONVERSOES...")
            progresso.progress(0.4)
            
            df_log_conversoes = conn.read(worksheet="LOG_CONVERSOES", ttl=0)
            
            conversoes_teste = [
                {
                    'ID_Conversao': gerar_id_unico('CONV'),
                    'Data_Conversao': '16/12/2025 10:00',
                    'Nome_Cliente': 'João Silva',
                    'Telefone': '11999999001',
                    'Classificacao_Cliente': 'Novo',
                    'Valor_Antes': 100.00,
                    'Valor_Depois': 180.00,
                    'Diferenca_Valor': 80.00,
                    'Compras_Antes': 1,
                    'Compras_Depois': 2,
                ...
                {
                    'ID_Conversao': gerar_id_unico('CONV'),
                    'Data_Conversao': '17/12/2025 11:30',
                    'Nome_Cliente': 'Maria Santos',
                    'Telefone': '11999999002',
                    'Classificacao_Cliente': 'Promissor',
                    'Valor_Antes': 250.00,
                    'Valor_Depois': 320.00,
                    'Diferenca_Valor': 70.00,
                    'Compras_Antes': 2,
                    'Compras_Depois': 3,
                    'Dias_Desde_Ultimo_Checkin': 1
                },
                {
                    'ID_Conversao': gerar_id_unico('CONV'),
                    'Data_Conversao': '17/12/2025 16:00',
                    'Nome_Cliente': 'Pedro Costa',
                    'Telefone': '11999999003',
                    'Classificacao_Cliente': 'Leal',
                    'Valor_Antes': 500.00,
                    'Valor_Depois': 650.00,
                    'Diferenca_Valor': 150.00,
                    'Compras_Antes': 5,
                    'Compras_Depois': 6,
            ...
            df_conv_atualizado = pd.concat([df_log_conversoes, pd.DataFrame(conversoes_teste)], ignore_index=True)
            conn.update(worksheet="LOG_CONVERSOES", data=df_conv_atualizado)
            st.success("✅ LOG_CONVERSOES populado com 3 conversões")
            
            time.sleep(0.5)
            
            # ========== 3. POPULAR METAS_DIARIAS ==========
            status.write("🎯 3/5 - Populando METAS_DIARIAS...")
            progresso.progress(0.6)
            ...
            df_metas_atualizado = pd.concat([df_metas, pd.DataFrame(metas_teste)], ignore_index=True)
            conn.update(worksheet="METAS_DIARIAS", data=df_metas_atualizado)
            st.success("✅ METAS_DIARIAS populado com 3 dias")
            
            time.sleep(0.5)
            
            # ========== 4. POPULAR HISTORICO_METRICAS ==========
            status.write("📊 4/5 - Populando HISTORICO_METRICAS...")
            progresso.progress(0.8)
            ...
            metricas_teste = [
                {
                    ...
                    'Meta_Dia': 28,
                    'Meta_Atingida': 'NAO',
                    'Agendamentos_Criados': 12,
                    'Agendamentos_Concluidos': 8,
                    'Tickets_Abertos': 2,
                    'Tickets_Resolvidos': 1,
                    'Tickets_Pendentes': 5,
                    'Conversoes_Dia': 1,
                    'Valor_Total_Novo': 4500.00,
                    'Valor_Total_Promissor': 8000.00,
                    'Valor_Total_Leal': 14000.00,
                    'Valor_Total_Campeao': 18000.00,
                    'Valor_Total_EmRisco': 2700.00,
                    'Valor_Total_Dormente': 3300.00,
                    'Valor_Total_Geral': 50500.00,
                    'Ticket_Medio_Dia': 315.63,
                    'Maior_Venda_Dia': 1200.00,
                    'Menor_Venda_Dia': 50.00
                },
                {
                    'Data': '16/12/2025',
                    'Total_Novo': 47,
                    ...
                    'Total_Dormente': 20,
                    'Total_Clientes': 163,
                    'CheckIns_Realizados': 32,
                    'Meta_Dia': 30,
                    'Meta_Atingida': 'SIM',
                    'Agendamentos_Criados': 32,
                    'Agendamentos_Concluidos': 28,
                    'Tickets_Abertos': 1,
                    'Tickets_Resolvidos': 2,
                    'Tickets_Pendentes': 4,
                    'Conversoes_Dia': 2,
                    'Valor_Total_Novo': 4700.00,
                    'Valor_Total_Promissor': 8500.00,
                    'Valor_Total_Leal': 14500.00,
                    'Valor_Total_Campeao': 19200.00,
                    'Valor_Total_EmRisco': 2550.00,
                    'Valor_Total_Dormente': 3000.00,
                    'Valor_Total_Geral': 52450.00,
                    'Ticket_Medio_Dia': 321.91,
                    'Maior_Venda_Dia': 1500.00,
                    'Menor_Venda_Dia': 80.00
                },
                {
                    'Data': '17/12/2025',
                    'Total_Novo': 48,
                    ...
                    'Meta_Dia': 28,
                    'Meta_Atingida': 'NAO',
                    'Agendamentos_Criados': 18,
                    'Agendamentos_Concluidos': 15,
                    'Tickets_Abertos': 0,
                    'Tickets_Resolvidos': 1,
                    'Tickets_Pendentes': 3,
                    'Conversoes_Dia': 3,
                    'Valor_Total_Novo': 4800.00,
                    'Valor_Total_Promissor': 8750.00,
                    'Valor_Total_Leal': 15000.00,
                    'Valor_Total_Campeao': 19200.00,
                    'Valor_Total_EmRisco': 2400.00,
                    'Valor_Total_Dormente': 2850.00,
                    'Valor_Total_Geral': 53000.00,
                    'Ticket_Medio_Dia': 323.17,
                    'Maior_Venda_Dia': 1200.00,
                    'Menor_Venda_Dia': 60.00
                }
            ]
            
            df_metricas_atualizado = pd.concat([df_historico_metricas, pd.DataFrame(metricas_teste)], ignore_index=True)
            conn.update(worksheet="HISTORICO_METRICAS", data=df_metricas_atualizado)
            st.success("✅ HISTORICO_METRICAS populado com 3 dias de dados")
            
            time.sleep(0.5)
            
            # ========== 5. VERIFICAÇÃO FINAL ==========
            status.write("✅ 5/5 - Verificando população...")
            progresso.progress(1.0)
            
            st.markdown("---")
            st.success("🎉 **POPULAÇÃO COMPLETA COM SUCESSO!**")
            st.balloons()
            
            st.markdown("### 📊 Resumo do que foi criado:")
            st.write("✅ **LOG_CHECKINS:** 5 check-ins de teste")
            st.write("✅ **LOG_CONVERSOES:** 3 conversões de teste")
            st.write("✅ **METAS_DIARIAS:** 3 dias de metas")
            st.write("✅ **HISTORICO_METRICAS:** 3 dias de snapshots completos")
            
            st.markdown("---")
            st.info("🚀 **Agora você pode criar o Dashboard! Há dados suficientes para todas as 21 métricas!**")
            
            st.markdown("### 🔍 Verifique no Google Sheets:")
            st.code("""
            ✅ LOG_CHECKINS - Deve ter 5+ linhas
            ✅ LOG_CONVERSOES - Deve ter 3+ linhas  
            ✅ METAS_DIARIAS - Deve ter 3+ linhas
            ✅ HISTORICO_METRICAS - Deve ter 3+ linhas
            """)
            
        except Exception as e:
            st.error(f"❌ Erro durante população: {e}")
            import traceback
